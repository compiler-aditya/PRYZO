"""
ElevenLabs Developer Journey Scraper
======================================
Scrapes 20 pages covering the full developer journey:
auth → models → TTS → streaming → WebSockets → STT →
voice cloning → webhooks → conversational AI → SDKs
"""
...
OUTPUT_DIR = os.path.join(SCRIPT_DIR, "elevenlabs_dev_journey")

EXCLUDE_TAGS = ["nav", "footer", "header", "aside", ".sidebar", ".breadcrumb", ".pagination"]

# ──────────────────────────────────────────────
# Developer journey: 20 most important pages
# ──────────────────────────────────────────────
TARGET_URLS = [
    # Foundation
    ...
    "https://elevenlabs.io/docs/eleven-api/guides/cookbooks/text-to-speech/streaming",
...
def main():
    ...
    print("\nTarget pages (developer journey):")
    for i, url in enumerate(TARGET_URLS, 1):
