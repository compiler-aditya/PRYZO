...
export default function Home() {
  ...
  return (
    <div className="space-y-12">
      {/* Hero Section */}
      <div className="relative w-full rounded-xl overflow-hidden bg-surface-container-low min-h-[300px] flex items-center p-8 md:p-12 vinyl-glow animate-fade-in">
        <div className="relative z-10 max-w-lg space-y-6">
          <h2 className="text-4xl md:text-6xl font-extrabold tracking-tighter leading-tight">
            Press play on <br />someone&apos;s{" "}
            <span className="text-primary-container animate-text-glow">truth</span>.
          </h2>
          <p className="text-on-surface-variant/60 text-sm md:text-base max-w-sm animate-fade-in-up" style={{ animationDelay: "200ms" }}>
            Anonymous stories, produced as intimate audio experiences. Every voice matters. No name needed.
          </p>
          <Link
            href="/submit"
            className="inline-flex items-center gap-2 bg-primary-container text-on-primary px-8 py-4 rounded-full font-bold hover:scale-105 active:scale-95 transition-all ripple-effect animate-fade-in-up"
        ...
        {/* Rotating Vinyl Art */}
        <div className="absolute right-[-10%] top-[-20%] w-[400px] h-[400px] md:w-[500px] md:h-[500px] opacity-20">
        ...
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-end gap-1 opacity-20">
      ...
      {/* Category Filter */}
      <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-none animate-fade-in-up" style={{ animationDelay: "300ms" }}>
        {CATEGORIES.map((cat) => (
          <button
            key={cat}
            onClick={() => setCategory(cat)}
            className={`px-5 py-2 rounded-full text-xs font-bold uppercase tracking-wide whitespace-nowrap transition-all duration-300
              ${category === cat
              ...
              {featured && (
                <div className="xl:col-span-2 space-y-6 animate-fade-in-up">
                  ...
                  <Link href={`/story/${featured.id}`} className="block group">
                    <div className={`glass-player rounded-xl p-6 md:p-8 border border-outline-variant/10 shadow-2xl relative overflow-hidden card-hover emotion-${featured.emotion || "wonder"} emotion-glow`}>
                      <div className="flex flex-col md:flex-row gap-6 items-center relative z-10">
                        ...
                        <div className="flex-1 space-y-3 w-full text-center md:text-left">
                          <div>
                            <h4 className="text-2xl md:text-3xl font-bold tracking-tight group-hover:text-primary transition-colors duration-300">
                              {featured.title}
                            </h4>
                          ...
                          <div className="flex items-center gap-4 justify-center md:justify-start pt-2">
                            <div className="w-14 h-14 rounded-full bg-primary-container text-on-primary flex items-center justify-center shadow-lg group-hover:scale-110 group-hover:shadow-primary-container/40 transition-all duration-300">
